...
import { backendUrl, websiteUrl } from "@/constants/myapi"
...
const AppHeader: React.FC = () => {
  ...
  const [tradeWorkingTime, setTradeWorkingTime] = useState<any>(null)
  ...
  const fetchTradeWorkingTime = async () => {
    try {
      setLoading(true)
      const userId = localStorage.getItem("user_id")
      const response = await axios.get(`${backendUrl}trade_working_time?user_id=${userId}`, {
        headers: {
      ...
      setTradeWorkingTime(response.data)
    } catch (error) {
      console.error("Error fetching trade working time:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <header className="sticky top-0 left-0 min-h-20 flex w-full  shadow border-gray-200  dark:border-gray-800 dark:bg-background lg:border-b z-99999 bg-white">
      <div className="flex flex-col items-center justify-between flex-grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3  border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          <button
          ...
          <div className="hidden lg:block">
            <form>
              <div className="relative">
                ...
                <input
                  ref={inputRef}
                  className="dark:bg-dark-900 bg-white h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring focus:ring-brand-500/10 dark:border-gray-800 dark:bg-tableBg  dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[430px]"
        ...
        <div
          ...
          className={`${
            isApplicationMenuOpen ? "flex py-4" : "hidden py-0"
          } items-center justify-between w-full md:gap-4 px-2 md:px-5  lg:flex shadow-theme-md lg:justify-end lg:px-0 lg:shadow-none `}
        >
          <div className="flex items-center gap-2 2xsm:gap-4 ">
            <NewNotificationDropDown tradeWorkingTime={tradeWorkingTime} />
            {/* <!-- Dark Mode Toggler --> */}
            {/* <ThemeToggleButton /> */}
            {/* <!-- Dark Mode Toggler --> */}

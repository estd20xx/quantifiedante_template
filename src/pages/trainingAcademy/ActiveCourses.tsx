...
interface ActiveCourseInterface {
  image: string
  title: string
  author: string
...
const activeCourses: Array<ActiveCourseInterface> = [
  {
  ...
  return (
    <motion.div className="w-full h-auto ">
      <div className="w-full flex md:items-center justify-between flex-col md:flex-row gap-5">
        <p className="text-[28px] font-medium text-start">Active Courses</p>
...
const CourseCard = ({ course }: Readonly<{ course: ActiveCourseInterface }>) => {
  return (
    <motion.div
      ...
      <div className="">
        <p
          className="mb-2 py-1 px-2 rounded text-sm text-grayuserText
              "
        >
          {course.currentLesson} of {course.totalLessons} Lessons Completed (
          {calculateProgress(course.totalLessons, course.currentLesson)}% )
        </p>
        <Progress
          ...
          size="md"
          value={calculateProgress(course.totalLessons, course.currentLesson)}
        />
      </div>

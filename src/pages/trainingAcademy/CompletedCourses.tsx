...
interface CompletedCourseInterface {
  image: string
  title: string
  author: string
...
const completedCourses: Array<CompletedCourseInterface> = [
  {
  ...
  return (
    <motion.div className="w-full h-auto ">
      <div className="w-full flex md:items-center justify-between flex-col md:flex-row gap-5">
        <p className="text-[28px] font-medium text-start">Complete Courses</p>
      ...
      <motion.div className=" w-full h-auto  grid grid-cols-1 xl:grid-cols-3 md:grid-cols-2 gap-6 my-5">
        {completedCourses.map((course, idx) => {
          return <CourseCard key={`${course.author}-${idx}`} completedCourses={course} />
        })}
      </motion.div>
...
const CourseCard = ({ completedCourses }: Readonly<{ completedCourses: CompletedCourseInterface }>) => {
  return (
    <motion.div
    ...
    >
      <motion.img
        alt={completedCourses.image}
        src={completedCourses.image}
        initial={{ scale: 0.95, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.1, duration: 0.4 }}
      />
      <p className="font-medium text-lg">{completedCourses.title}</p>
      <p className="text-sm">
        <span className="text-grayuserText">by</span> {completedCourses.author}
      </p>
      <p
        className="bg-graySkeleton py-1 px-2 rounded text-xs text-blackDarkGray
              "
      >
        {completedCourses.currentLesson} Lessons
        <span className="text-grayuserText"> | 0 of {completedCourses.totalLessons} Lessons Completed</span>
      </p>
      <p className="text-sm font-normal">{completedCourses.descriptions}</p>
      <div className="">
        <p
          className="mb-2 py-1 px-2 rounded text-sm text-grayuserText
              "
        >
          {completedCourses.currentLesson} of {completedCourses.totalLessons} Lessons Completed (
          {calculateProgress(completedCourses.totalLessons, completedCourses.currentLesson)}% )
        </p>
        <Progress
          ...
          size="md"
          value={calculateProgress(completedCourses.totalLessons, completedCourses.currentLesson)}
        />
      </div>
      ...
      <button className="rounded border border-pinkMilkish w-full text-pinkMilkish whitespace-nowrap py-3 px-4 text-sm hover:opacity-70 duration-300">
        Loarn More
      </button>
    </motion.div>
  )
